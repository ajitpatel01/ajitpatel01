...
Usage: ./scripts/build.sh [flags]
  ...
  (default)     prep photo → ascii → info card → heatmap
...
if [[ $# -gt 0 ]]; then
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --info) DO_INFO=1 ;;
      --heatmap) DO_HEAT=1 ;;
      --no-prep) DO_PREP=0; DO_ASCII=1; DO_INFO=1; DO_HEAT=1 ;;
      -h|--help) usage; exit 0 ;;
...
echo "==> done"
ls -la ajit-ascii.svg info-card.svg contrib-heatmap.svg 2>/dev/null || true
